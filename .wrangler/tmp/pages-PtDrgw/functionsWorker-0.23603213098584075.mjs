...
// lib/db.ts
async function findAccountByName(db, pokerName) {
  return await db.prepare("SELECT * FROM accounts WHERE poker_name = ?").bind(pokerName).first();
}
__name(findAccountByName, "findAccountByName");
async function findAccountById(db, id) {
  return await db.prepare("SELECT * FROM accounts WHERE id = ?").bind(id).first();
}
__name(findAccountById, "findAccountById");
async function listAccounts(db) {
  const res = await db.prepare("SELECT * FROM accounts ORDER BY id ASC").all();
  return res.results ?? [];
}
__name(listAccounts, "listAccounts");
async function insertAccount(db, args) {
  const now = Date.now();
  const res = await db.prepare(
    "INSERT INTO accounts (poker_name, private_pass, is_admin, created_at) VALUES (?, ?, ?, ?)"
  ).bind(args.pokerName, args.privatePass, args.isAdmin ? 1 : 0, now).run();
...
__name(insertAccount, "insertAccount");
async function touchLastLogin(db, accountId) {
  await db.prepare("UPDATE accounts SET last_login_at = ? WHERE id = ?").bind(Date.now(), accountId).run();
}
__name(touchLastLogin, "touchLastLogin");
async function createSession(db, args) {
  const id = crypto.randomUUID();
  const now = Date.now();
  const expiresAt = now + args.durationMs;
  await db.prepare(
    "INSERT INTO sessions (id, account_id, created_at, expires_at) VALUES (?, ?, ?, ?)"
  ).bind(id, args.accountId, now, expiresAt).run();
  return { id, expiresAt };
}
__name(createSession, "createSession");
async function findActiveSession(db, sessionId) {
  const now = Date.now();
  return await db.prepare("SELECT * FROM sessions WHERE id = ? AND expires_at > ?").bind(sessionId, now).first();
}
...
async function findActiveGroupKey(db) {
  return await db.prepare("SELECT * FROM group_keys WHERE active_until IS NULL ORDER BY id DESC LIMIT 1").first();
}
__name(findActiveGroupKey, "findActiveGroupKey");
async function rotateGroupKey(db, newKeyValue) {
  const now = Date.now();
  await db.batch([
...
__name(rotateGroupKey, "rotateGroupKey");
...
async function resolveAccountFromSession(db, sessionId) {
  const session = await findActiveSession(db, sessionId);
  if (!session) return null;
  return await findAccountById(db, session.account_id);
}
__name(resolveAccountFromSession, "resolveAccountFromSession");
async function resolveAccountFromRequest(db, request) {
  const token = extractBearerToken(request.headers.get("Authorization"));
  if (!token) return null;
  return await resolveAccountFromSession(db, token);
}
__name(resolveAccountFromRequest, "resolveAccountFromRequest");
function toAccountPublic(row) {
  return {
    id: row.id,
...
__name(toAccountPublic, "toAccountPublic");
...
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (typeof newKey !== "string" || newKey.length < KEY_MIN || newKey.length > KEY_MAX) {
    return jsonResponse(400, { error: "invalid_payload" });
  }
...
// api/admin/group_keys.ts
var onRequestGet2 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const me = await resolveAccountFromRequest(env.DB, request);
  if (!me || me.is_admin !== 1) {
    return jsonResponse(403, { error: "forbidden" });
  }
  const res = await env.DB.prepare("SELECT * FROM group_keys ORDER BY active_from DESC").all();
...
var onRequestPost2 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (typeof pokerName !== "string" || typeof privatePass !== "string" || typeof groupKey !== "string" || pokerName.length === 0 || privatePass.length === 0 || groupKey.length === 0) {
    return jsonResponse(400, { error: "invalid_payload" });
  }
...
// api/auth/signup.ts
var POKER_NAME_MAX = 32;
var PRIVATE_PASS_MAX = 128;
var onRequestPost4 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const privatePass = body.private_pass;
  const groupKey = body.group_key;
  if (typeof pokerName !== "string" || typeof privatePass !== "string" || typeof groupKey !== "string" || pokerName.length === 0 || pokerName.length > POKER_NAME_MAX || privatePass.length === 0 || privatePass.length > PRIVATE_PASS_MAX || groupKey.length === 0) {
    return jsonResponse(400, { error: "invalid_payload" });
  }
  const activeKey = await findActiveGroupKey(env.DB);
  if (!activeKey || !constantTimeEqual(activeKey.key_value, groupKey)) {
...
// ../.wrangler/tmp/pages-PtDrgw/functionsRoutes-0.8601345032259319.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-oA4TLc/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-oA4TLc/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.23603213098584075.mjs.map
