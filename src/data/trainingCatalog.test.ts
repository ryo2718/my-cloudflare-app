...
describe('TRAINING_CATALOG', () => {
  ...
  it('preflop 7 レベル, flop 6 レベル (初級/中級CB/中級Donk/中級BMCB/上級/超上級)', () => {
    ...
    expect(TRAINING_CATALOG[1].levels.map((l) => l.key)).toEqual([
      'flop_beginner',
      'flop_intermediate',
      'flop_intermediate_donk',
      'flop_intermediate_bmcb',
      'flop_advanced',
      'flop_expert',
    ]);
  });
  ...
  it('preflop 上級/超上級 は false。flop は初級+中級CBが実装、Donk/BMCB/上級/超上級は未実装', () => {
    ...
    expect(TRAINING_CATALOG[1].levels[1].implemented).toBe(true);  // フロップ中級CB
    expect(TRAINING_CATALOG[1].levels.slice(2).every((l) => l.implemented === false)).toBe(true);
  });

  it('flop: 初級20問 / 中級CB30問、Donk以降は questionCount=null', () => {
    expect(TRAINING_CATALOG[1].levels[0].questionCount).toBe(20); // 初級
    expect(TRAINING_CATALOG[1].levels[1].questionCount).toBe(30); // 中級CB
    expect(TRAINING_CATALOG[1].levels[1].timeLimitSec).toBe('none');
...
describe('helpers', () => {
  it('isPlanned: questionCount !== null は planned', () => {
    ...
    expect(isPlanned(TRAINING_CATALOG[1].levels[1])).toBe(true);  // フロップ中級CB (計画済)
    expect(isPlanned(TRAINING_CATALOG[1].levels[2])).toBe(false); // フロップ中級Donk (未計画)
  });
  ...
  it('isPlayable: implemented=true かつ pt/問数あり', () => {
    ...
    expect(isPlayable(TRAINING_CATALOG[1].levels[1])).toBe(true);  // フロップ中級CB (実装済)
    expect(isPlayable(TRAINING_CATALOG[1].levels[2])).toBe(false); // フロップ中級Donk (未実装)
  });
  ...
  it('formatLevelInfo: フロップ中級CB は "30問・最大 60pt・制限時間なし"', () => {
...
describe('maxScoreFor', () => {
  ...
  it('フロップ中級CB: 60 (questionCount * 2)', () => {
  ...
  it('未計画 (questionCount=null) → 0', () => {
    ...
    expect(maxScoreFor(TRAINING_CATALOG[1].levels[2])).toBe(0); // フロップ中級Donk (未計画)
  });
});
