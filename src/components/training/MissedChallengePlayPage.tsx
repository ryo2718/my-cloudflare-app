// /quiz/review/preflop/{level}/play?count=N: 間違えた問題の挑戦モード。
...
interface PreparedBeginner {
  kind: 'beginner';
  missed_problem_id: number;
  question: PreflopQuestion;
}

interface PreparedIntermediate {
  kind: 'intermediate';
  missed_problem_id: number;
  question: IntermediateQuestion;
}

type Prepared = PreparedBeginner | PreparedIntermediate;
...
export function MissedChallengePlayPage({ level, count }: Props) {
  const auth = useAuth();
  ...
  useEffect(() => {
    ...
    apiGetMissedProblems(sid, { level, limit: 1000 })
      .then((rows) => {
        if (cancelled) return;
        // shuffle → take N → convert
        const taken = shuffle(rows).slice(0, count);
  ...
  }, [auth.sessionId, level, count]);

  const recordsRef = useRef<MissedChallengeItem[]>([]);
  // ready 状態の results を ref と同期 (advance / finish で利用)
  useEffect(() => {
    if (state.kind === 'ready') recordsRef.current = state.results;
  }, [state]);
  ...
  const advanceBeginner = (chosen: CorrectAnswer) => {
    ...
    if (item.kind !== 'beginner') {
      advancingRef.current = false;
      return;
    }
    const correct = item.question.correct;
    const isCorrect = correct === chosen;
    const finalScore = isCorrect ? 1 : -1;
    // 初級ペア missed_problems の row.id ← prepared から
    const missed_problem_id = item.missed_problem_id;
    const newItem: MissedChallengeItem = {
      missed_problem_id,
    ...
    const newResults = [...state.results, newItem];
  ...
  const advanceIntermediate = (selected: ReadonlyArray<Action>) => {
    ...
    const breakdown = scoreAnswer(item.question.strategy, selected);
  ...
  if (state.kind === 'loading') {
    return (
      <div style={pageStyle}>
        <div style={infoStyle}>問題を読み込み中…</div>
      </div>
    );
  }
  if (state.kind === 'error') {
    ...
    return (
      <div style={pageStyle}>
        <div style={errorBoxStyle}>
          ...
          <button
            type="button"
            onClick={() => navigate(`/quiz/review/preflop/${level}`)}
            style={errorBtnStyle}
          >
            戻る
          </button>
  ...
  return (
    <div style={pageStyle}>
      <header style={headerBarStyle}>
        <div style={progressTopStyle}>
          <span style={progressLabelStyle}>
            プリフロップ{level === 'beginner' ? '初級' : '中級'} 挑戦モード
          </span>
      ...
      <main style={mainStyle}>
        {item.kind === 'beginner' ? (
          <BeginnerStage q={item.question} onAnswer={advanceBeginner} />
        ) : (
...
function IntermediateStage({
  q,
  onAnswer,
}: {
  q: IntermediateQuestion;
  onAnswer: (sel: ReadonlyArray<Action>) => void;
}) {
  // ACTIONS は順序固定の参照 (variant lint 回避のため弱参照)
  void ACTIONS;
  const scenarioPill = useMemo(() => intermediateScenarioLabel(q), [q]);
  return (
    <>
      <div style={scenarioPillStyle}>{scenarioPill}</div>
      <PokerTable
        mePosition={q.myPosition}
...
// ---------------------------------------------------------------------------
// Styles (TrainingPlay / TrainingPlayIntermediate と概ね同一だが Countdown は無し)
// ---------------------------------------------------------------------------
