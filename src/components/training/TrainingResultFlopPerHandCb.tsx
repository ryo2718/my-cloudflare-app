// フロップ中級CB(個別ハンド) の結果画面。スコア + 正誤内訳 (○/✕) + 振り返り (各問展開でレンジgrid)。
// best_score は POST /api/account/training-result で保存 (training_type=flop_intermediate_cb)。
...
import { loadFlopPhRecords } from '../../data/training/flopPerHandRecordsStore';
import { flopPhScenarioLabel, type FlopPhRecord } from '../../data/training/flopPerHandCb';
...
export interface TrainingResultFlopPerHandCbProps {
  level: TrainingLevel;
}
...
export function TrainingResultFlopPerHandCb({ level }: TrainingResultFlopPerHandCbProps) {
  const auth = useAuth();
  const scoreInfo = parseQuery();
  const [records, setRecords] = useState<FlopPhRecord[]>(() => loadFlopPhRecords(level.key) ?? []);
  ...
  useEffect(() => {
    const r = loadFlopPhRecords(level.key);
          ...
          <section style={reviewSectionStyle} aria-label="振り返り一覧">
            ...
            <ul style={listStyle}>
              {records.map((r) => {
                ...
                const symbol = flopJudgment(r.points);
                ...
                return (
                  <li key={r.recordId} style={itemStyle}>
                    ...
                    >
                      ...
                      <span style={scenarioPillStyle}>{flopPhScenarioLabel(r)}</span>
                      <span style={handTagStyle}>{r.hand}</span>
...
const scenarioPillStyle: CSSProperties = {
  fontSize: '0.7rem', fontWeight: 700, color: '#993C1D', background: '#FAEEDA',
  border: '1px solid #E5A551', borderRadius: '999px', padding: '0.15rem 0.5rem', whiteSpace: 'nowrap',
};
const handTagStyle: CSSProperties = { fontFamily: 'ui-monospace, monospace', fontSize: '0.82rem', fontWeight: 800, color: THEME.textPrimary };
...
const detailWrapStyle: CSSProperties = { padding: '0.7rem', borderTop: `1px solid ${THEME.border}`, background: '#FCFBF8', display: 'flex', flexDirection: 'column', gap: '0.6rem' };
